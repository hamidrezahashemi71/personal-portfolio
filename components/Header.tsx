import React from "react";
import {SocialIcon} from "react-social-icons";
import {motion} from "framer-motion";
import Link from "next/link";

type Props = {};

const Header = (props: Props) => {
  return (
    <header className='sticky top-0 p-5 flex items-start xl:items-center justify-between max-w-7xl mx-auto z-20'>
      <motion.div
        initial={{x: -500, opacity: 0, scale: 0.5}}
        animate={{x: 0, opacity: 1, scale: 1}}
        transition={{duration: 1.5}}
        className='flex flex-row items-center'>
        <SocialIcon
          url='https://www.linkedin.com/in/hh7192/'
          fgColor='gray'
          bgColor='transparent'
        />
        <SocialIcon
          url='https://api.whatsapp.com/send?phone=989195892013'
          fgColor='gray'
          bgColor='transparent'
        />
        <SocialIcon
          url='https://t.me/Hamidreza9'
          fgColor='gray'
          bgColor='transparent'
        />
        <SocialIcon
          url='https://github.com/hamidrezahashemi71'
          fgColor='gray'
          bgColor='transparent'
        />
      </motion.div>

        <motion.div
          initial={{x: 500, opacity: 0, scale: 0.5}}
          animate={{x: 0, opacity: 1, scale: 1}}
          transition={{duration: 1.5}}
          className='flex flex-row items-center text-gray-300 cursor-pointer'>
          <SocialIcon
            className='cursor-pointer'
            network='email'
            fgColor='gray'
            bgColor='transparent'
          />
          <Link href={"#contact"}>
            <p className='hidden md:inline-flex uppercase text-sm text-gray-400'>
             Get in Touch
            </p>
           </Link>
        </motion.div>
    </header>
  );
};

export default Header;
